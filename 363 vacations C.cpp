...
int call(int i,int p){
    ...
    int ret = 0;
    //ret = call(i+1,0);
    if(a[i] == 1 && p != 1)   ret = max(ret,call(i+1,1)+1);
    if(a[i] == 2 && p != 2)   ret = max(ret,call(i+1,2)+1);
    if(a[i] == 3){
        if(p != 2){
            ret = max(ret,call(i+1,2)+1);
        }
        if(p != 1){
            ret = max(ret,call(i+1,1)+1);
        }
    }
    ret = max(ret,call(i+1,0));
    return dp[i][p] = ret;
}
